...
# Utility function for Hungarian algorithms
...
class ClassicalBenchmark:
    """
    Runs classical exploration algorithms benchmark on multiple seeds.
    Extracts metrics at 90% coverage and generates raw + aggregated CSV outputs.
    """
    ...
    def run_benchmark(self):
        ...
        print(f"Running benchmark on seeds {START_SEED}-{END_SEED}")
    ...
    def save_raw_csv(self):
        ...
        """
        output_dir = os.path.join("results", "classical_benchmark")
        os.makedirs(output_dir, exist_ok=True)

        filename = os.path.join(output_dir, "raw_result_test_new.csv")
    ...
    def print_aggregated_results(self):
        ...
        # Print header
        header = f"{'Strategy':<25} | {'Time to 90%':<12} | {'Distance':<12} | {'Sensing Redundancy':<18} | {'Visit Overlap':<14} | {'Movement Efficiency':<18} | {'Nodes Expanded':<14}"
        print(header)
        print("-" * 100)
        ...
        for strategy_name, stats in self.aggregated_results.items():
            row = (
                ...
                f"{stats['nodes_expanded_mean']:.0f} ± {stats['nodes_expanded_std']:.0f}<{6}"
            )
            print(row)
